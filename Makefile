.PHONY: backend-dev frontend-dev dev pipeline pipeline-live pipeline-sample backtest price-sync price-sync-loop test setup-venv install-backend install-frontend build-frontend serve
...
serve: build-frontend
	INTEGRITY_DB_PATH="$(INTEGRITY_DB_PATH)" "$(PYTHON)" -m uvicorn app.main:app --port 8000 --app-dir backend
